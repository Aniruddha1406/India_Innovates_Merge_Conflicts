...
function DirectionPanel({ direction, signalState }) {
    ...
    return (
        <div style={{ position: 'relative', overflow: 'hidden', background: '#0a0f18' }}>
            {/* Real YOLO MJPEG stream from the pipeline */}
            {streamOk ? (
                <img
                    src={streamUrl}
                    alt={`CAM: ${direction.id} — YOLO Live`}
            ...
            {/* Direction label + density */}
            <div style={{ position: 'absolute', top: 8, left: 8, background: 'rgba(0,0,0,0.85)', padding: '5px 12px', borderRadius: 6, border: `1px solid ${direction.color}55`, zIndex: 3, backdropFilter: 'blur(4px)' }}>
                <div style={{ color: direction.color, fontSize: '0.65rem', fontWeight: 900, letterSpacing: '0.05em' }}>CAM: {direction.id}</div>
                <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.45rem', marginTop: 2 }}>
                    {direction.axisLabel} Density: {density}%  ·  {vehicles} vehicles
                </div>
            </div>

            {/* Live signal color indicator top-right */}
            <div style={{ position: 'absolute', top: 8, right: 8, zIndex: 3, display: 'flex', alignItems: 'center', gap: 5,
                background: `${SIG_COLOR[dirSignal]}22`, border: `1px solid ${SIG_COLOR[dirSignal]}66`,
                borderRadius: 6, padding: '3px 8px', backdropFilter: 'blur(4px)' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: SIG_COLOR[dirSignal], boxShadow: SIG_GLOW[dirSignal], animation: 'pulse-dot 1.2s infinite' }} />
                <span style={{ fontSize: '0.52rem', fontWeight: 800, fontFamily: 'monospace', color: SIG_COLOR[dirSignal] }}>
                    {dirSignal.toUpperCase()}
                </span>
            </div>

            {/* REC badge */}
            <div style={{ position: 'absolute', bottom: 6, left: 6, display: 'flex', alignItems: 'center', gap: 3, zIndex: 3 }}>
                <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#ff3b5c', animation: 'pulse-dot 1.2s infinite' }} />
                <span style={{ fontSize: '0.38rem', fontWeight: 800, color: '#ff3b5c', fontFamily: 'monospace', letterSpacing: '0.06em' }}>REC</span>
            </div>

            {/* Green signal flash overlay */}
            {dirSignal === 'green' && (
                <div style={{ position: 'absolute', inset: 0, border: '2px solid rgba(0,255,157,0.5)', boxShadow: 'inset 0 0 20px rgba(0,255,157,0.15)', pointerEvents: 'none', zIndex: 4, borderRadius: 0 }} />
            )}
        </div>
    );
}
